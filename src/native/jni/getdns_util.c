 ...
 */
jobject
convertBinData(JNIEnv* env, getdns_bindata* data,
                    const char* key) {
    ...
    if (data->size == 1 && data->data[0] == 0) {
        ...
        if (getdns_convert_dns_name_to_fqdn(data, &dnameOrIp)
            != GETDNS_RETURN_GOOD) {
            fprintf(stderr, "getdns_convert_dns_name_to_fqdn != GETDNS_RETURN_GOOD for the key: %s\n", key);
        }
    } else if (key != NULL &&
        ...
        result = convertUnsignedCharArrayToByteArray(env, data->data, data->size);
    }
    ...
    for (i = 0; i < len; ++i) {
        ...
        switch (type) {
            case t_bindata:
            {
		        ...
		        (*env)->CallObjectMethod(env, resultslist, methods->listAdd, convertedData);
                }
                break;
            }
        ...
        switch (type) {
            case t_bindata:
            {
                if(getdns_dict_get_bindata(dict, (char*)nameBin->data, &data) == GETDNS_RETURN_GOOD) {
                    convertedData = convertBinData(env, data, (char*)nameBin->data);
                    if(convertedData != NULL)
                        (*env)->CallObjectMethod(env, responseObject, methods->mapPut, key, convertedData);
		}
                break;
            }
            case t_int:
...
getdns_list* convertToGetDNSList
  (JNIEnv *env, jobject thisObj, jobject mapObj) {
    ...
    jobject jiterator = (*env)->CallObjectMethod(env, mapObj, methods.listGetIterator);
    size_t idx = 0;
    ...
    while ((*env)->CallBooleanMethod(env, jiterator, methods.hasNext)) {
	    ...
	    getdns_list_set_int(result, idx, (*env)->CallIntMethod(env, jvalue, methods.intValue));
        }
            ...
            getdns_bindata bdata = {(*env)->GetStringUTFLength(env, jvalue), (char*)value};
        ...
        else if((*env)->IsAssignableFrom(env, (*env)->FindClass(env, "java/util/List"), valueClass)) { // Map value case.
...
getdns_dict* convertMapToDict
  (JNIEnv *env, jobject thisObj, jobject mapObj) {
	...
	const char* key = (*env)->GetStringUTFChars(env, jkey, 0);
        ...
        else if((*env)->IsAssignableFrom(env, (*env)->FindClass(env, "java/util/ArrayList"), valueClass)) { // Map value case.
            getdns_list* sublist = convertToGetDNSList(env, thisObj, jvalue);
